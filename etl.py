...
def process_song_file(cur, filepath):
    """ Use song data to insert data into two dimension tables (songs, artists). """
    # open song file
...
def process_log_file(cur, filepath):
    """ Use log data to insert data into two dimension talbes (times, users) and one fact table (songplay). """
    # open log file
    df = pd.read_json(filepath, lines=True)
    ...
    time_data = {'start_time': t.dt.time,
            'hour': t.dt.hour, 
            'day': t.dt.day, 
...
#     column_labels = 
    ...
    for i, row in time_df.drop_duplicates().iterrows():
        cur.execute(time_table_insert, list(row))

    # load user table
    user_df = df.filter(items=['userId', 'firstName', 'lastName', 'gender', 'level'], axis = 1)
...
def main():
    ...
    Insert into dimension and fact tables by calling a different process fuction.
